<template>
	<div>
				...
				<li>
					<el-button-group >
						...
						<el-upload style="display: inline;" :show-file-list="uplist" class="upload-demo" action="https://jsonplaceholder.typicode.com/posts/"
							:on-preview="handlePreview" :on-remove="handleRemove" :before-remove="beforeRemove" multiple :limit="3" :on-exceed="handleExceed"
							:file-list="fileList">
				...
				<el-tab-pane label="项目详情" name="first">
					...
					<vxe-form :data="formdata" size="medium" :span="12" title-align="left" title-width="135px" ref="xmxxform" :rules="rules">
						<vxe-form-item title="项目名称" field="xmmc">
							<template v-slot>
								<vxe-input v-model="formdata.xmmc" placeholder=" "></vxe-input>
							</template>
						</vxe-form-item>
						...
						<vxe-form-item title="项目标签1" field="bqjzclbdsyicode" v-show="extral3">
							<template v-slot>
								<vxe-input v-model="formdata.bqjzclbdsyicode" placeholder=" "></vxe-input>
							</template>
						</vxe-form-item>
						...
						<vxe-form-item title="建设单位" v-show="extral1">
							<template v-slot>
								<vxe-input v-model="formdata.jsdw" placeholder=" "></vxe-input>
							</template>
						</vxe-form-item>
						...
						<vxe-form-item title="www " :folding="folding">
							<template v-slot>
								<vxe-input v-model="formdata.id" placeholder=" "></vxe-input>
							</template>
						</vxe-form-item>
					</vxe-form>
				</el-tab-pane>
				<el-tab-pane label="预算情况表" name="second" :disabled="tabsec">
					...
					<div class="row">
						<vxe-grid border stripe resizable height="500" :columns="tableColumn" :data="tableData1" ref="ysqkgrid"
						 :edit-config="{trigger: 'click', mode: 'cell'}" @edit-closed="inputchange" :highlight-current-row="highlight">
						</vxe-grid>
					</div>
				</el-tab-pane>
			</el-tabs>
		...
		<el-dialog title="提示" :visible.sync="xmkndsxdialog" width="25%">
			<span>{{xmkndsxmessage}}</span>
			<span slot="footer" class="dialog-footer">
				<el-button @click="xmkndsxdialogcan">取 消</el-button>
				<el-button type="primary" @click="xmkndsxdialogyes">确 定</el-button>
			</span>
		</el-dialog>
...
<script>
	...
	export default {
		...
		methods: {
			...
			xmkndsxdialogcan() {
				this.xmkndsxdialog = false;
				this.formdata.xmkndsxid = null
				this.extral2 = false
			},
			xmkndsxdialogyes() {
				this.xmkndsxdialog = false;
			...
			inputchange(item) {
				...
				sum.plus2=Number(sum.zcchuzusr || 0) +Number(sum.zcchuzhisr || 0) +Number(sum.zcdwtzsy || 0) +Number(sum.zczczr || 0) 
			...
			edit() {
				...
				api.getysxmbyid(param).then(response => {
					...
					var xmlb = this.formdata.xmlbcode;
					var xmkndsx = this.formdata.xmkndsxcode;
					if (xmlb == 3 || xmlb == 4) {
						this.extral1 = true;
					} else {
						this.extral1 = false;
			...
			ndsxchange(value) {
				...
				if (obj.code == 1 || obj.code == 2) {
					...
					this.xmkndsxmessage = '新建预算项目跨年度属性为跨年度项目？'
					this.xmkndsxdialog = true;
					//this.extral2 = true;
				}
			},
			xmndchange(value) {
